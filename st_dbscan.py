...
class ST_DBSCAN():
    ...
    def __init__(self,
                 ...
                 eps2=10,
                 min_samples=5,
                 metric_temporal='euclidean',
                 metric_spatial='euclidean',
                 n_jobs=-1):
        self.eps1 = eps1
        self.eps2 = eps2
        self.min_samples = min_samples
        self.metric_temporal = metric_temporal
        self.metric_spatial = metric_spatial
        self.n_jobs = n_jobs
    ...
    def fit(self, X):
        ...
        if len(X) < 20000:
            ...
            time_dist = pdist(X[:, 0].reshape(n, 1), metric=self.metric_temporal)

            if self.metric_spatial == "haversine":
                haversine_dist = haversine_distances(X[:, 1:])

                # filter the euc_dist matrix using the time_dist
                dist = np.where(squareform(time_dist) <= self.eps2, haversine_dist, 2 * self.eps1)
                ...
                euc_dist = pdist(X[:, 1:], metric=self.metric_spatial)
    ...
    def fit_frame_split(self, X, frame_size, min_frame_size=None, frame_overlap=None):
        """
        ...
        min_frame_size : float, default=frame_size
            Sets the minimum frame size for splitting
        ...
        if min_frame_size == None:
            min_frame_size = frame_size

        # a wrong amount of labels would otherwise be generated and overlap points would be calculated twice
        if min_frame_size < frame_overlap:
            raise ValueError('min_frame_size must be larger than frame_overlap')
        ...
        # fallback if data has less than min_frame_size
        if len(time) <= min_frame_size:
            frame_starts = [0]
            frame_ends = [len(time)]
        else:
            # with open("test.txt","w") as f:
            #     f.write("I totally crashed WJYUYYYYYYYYYoieqhoiqhsqrghogf")
            #     f.write(f"{type(frame_size)},{type(min_frame_size)},{type(frame_overlap)},{len(X)},{frame_size},{min_frame_size},{frame_overlap}")
            #print(type(frame_size),type(min_frame_size),type(frame_overlap),len(X),frame_size,min_frame_size,frame_overlap)
            frame_starts = list(range(0, (len(time) - min_frame_size), (frame_size - frame_overlap + 1)))
            frame_ends = [start + frame_size for start in frame_starts]

            # fixes last frame end to be within the dataset bounds
            frame_ends[-1] = len(time)

        for start,end in zip(frame_starts, frame_ends):
            for period in [time[start:end]]:
    ...
    def fit_frame_split_new(self, X, min_frame_size):
        """
        Apply the ST DBSCAN algorithm with splitting it into frames.
        ----------
        X : 2D numpy array with
            ...
            [0,0.54,0.34],...])
        min_frame_size : float, default=frame_size
            Sets the minimum frame size for splitting, the max size of a frame depends on the dataset distribution
            The frame size is calculated making sure that this method returns exactly the same clusters as the fit method
        Returns
        -------
        self
        """
        ...
        if not min_frame_size > self.min_samples:
            raise ValueError(
                'min_frame_size not correctly configured. must be larger than min_samples')
        ...
        if len(time) < min_frame_size:
            ...
            frame_starts = [0]
            frame_ends = [min_frame_size]

            while frame_ends[-1] < len(time) - min_frame_size:
                tmp_overlap_start = frame_ends[-1] - self.min_samples
                tmp_frame_end = tmp_overlap_start + min_frame_size

                while (a:=(tmp_overlap_start != frame_starts[-1])) and (time[frame_ends[-1]-1] - time[tmp_overlap_start] <= 2*self.eps2):
                    tmp_overlap_start -= 1

                # if condition b is the reason for not increasing the overlap, always merge the frames
                if not a:
                    frame_ends[-1] = tmp_frame_end
                else:
                    # increases the end of the frame until at least a time range of 2*eps2 has passed in the frame
                    while (b:= (tmp_frame_end != len(time))) and (time[tmp_frame_end - 1] - time[tmp_overlap_start] <= 4*self.eps2):
                        tmp_frame_end += 1

                    # we cannot make a frame that satisfies all conditions, finalize the framing by putting the remaining data together
                    if not b:
                        frame_ends[-1] = len(time)
                    else:
                        frame_starts.append(tmp_overlap_start)
                        frame_ends.append(tmp_frame_end)


            # fixes last frame end to be within the dataset bounds
            frame_ends[-1] = len(time)

        for frame_index, (start,end) in enumerate(zip(frame_starts, frame_ends)):
            #period = time[start:end]
            period_start = time[start]
            period_end = time[end-1]
            overlap_time_end =  time[frame_ends[frame_index-1]-1]
            frame = X[(X[:, 0]>=period_start) & (X[:, 0]<=period_end)]
            ...
            if not type(labels) is np.ndarray:
                ...
                # needed for when there are duplicate timestamps
                #right_overlap = len(X[np.isin(X[:, 0], time[start:frame_ends[frame_index-1]])])
                right_overlap = np.sum((X[:, 0]>=period_start) & (X[:, 0]<=overlap_time_end))

                frame_one_overlap_labels = labels[len(labels) -
                                                  right_overlap:]
                frame_two_overlap_labels = self.labels[0:right_overlap]

                labels_counter = labels.max() + 1

                mapper = {}
                for i in list(
                        zip(frame_one_overlap_labels,
                            frame_two_overlap_labels)):
                    # never map to -1 if i[1]!=-1 but map to the next label_counter
                    if i[1] != -1:
                        if i[0] == -1:
                            if i[1] not in mapper:
                                mapper[i[1]] = labels_counter
                                labels_counter += 1
                        else:
                            mapper[i[1]] = i[0]
                ...
                # recode them to new cluster value
                for j in ignore_clusters:
                    # do not overwrite the -1 default (happens when overlap has no outliers)
                    if j != -1:
                        mapper[j] = labels_counter
                        labels_counter += 1
                ...
                # objects in the second frame are relabeled to match the cluster id from the first frame
                # objects in clusters with no overlap are assigned to new clusters
                # objects in the overlap that are outliers in the second frame only become outliers when they are so in the first frame
                new_labels = np.array([frame_one_overlap_labels[index] if index < right_overlap and j == -1 and frame_one_overlap_labels[index] != -1 else mapper[j] for index, j in enumerate(self.labels)])
